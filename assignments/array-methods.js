...
// of each runner for their running bibs.  
...
// ==== Challenge 2: Use .map() ====
// The event director needs to have all the 
// runner's first names converted 
// to uppercase because the director BECAME DRUNK WITH POWER. 
// Convert each first name into all caps and log the result

let allCaps = runners.map(r => {
	return {first_name: r.first_name.toUpperCase()}
})
...
// ordering issue.  Get a list of runners with 
...
// Now that you have used .forEach(), .map(), 
// .filter(), and .reduce().  I want you to think of 
// potential problems you could solve given the data set and 
// the 5k fun run theme.  Try to create and then solve 3 
// unique problems using one or many of the array methods listed above.

// Problem 1

//get all users whos donations are greater than 200

let highDonations = runners.filter(runner => {
...
// Problem 2
// getall users who donations are less than 30
let lowDonations = runners.filter(runner => {
	return runner.donation < 30
})
...
// Problem 3
// get an array that had all the names next to emails
let nameEmail = runners.map(runner => {
